...
export class CircularReferenceTypeFormatter implements SubTypeFormatter {
    ...
    /**
     * Recompute all cached definitions that may contain stale data from
     * circular-reference placeholders.
     *
     * During initial processing, when a recursive type (e.g. a class with
     * `fields: BindingNode[]`) is being processed, the cache returns {}
     * for it. All definitions computed during that window embed the
     * incomplete placeholder. This method recomputes every cached entry
     * using now-complete member caches.
     *
     * Every entry is checked for convergence because wrapper types
     * (AliasType, AnnotatedType, etc.) may sit between the stale source
     * and the consuming definition, and their cache entries must also
     * propagate the fix before the loop can exit.
     */
    public recomputeIntersections(options?: GetDefinitionOptions): void {
