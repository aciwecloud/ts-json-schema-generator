...
function resolveRefName(
    ...
    if (rec.anyOf && typeof rec.anyOf === "object") {
        const name = arrayToName.get(rec.anyOf as object);
        if (name) return name;
    }
    if (rec.oneOf && typeof rec.oneOf === "object") {
        const name = arrayToName.get(rec.oneOf as object);
        if (name) return name;
    }
    return undefined;
}
...
/**
 * Walk the schema definitions and replace circular JS object references
 * with JSON Schema `$ref` pointers.
 *
 * After recomputation with shallow copies, the cached definitions may contain
 * circular JS object references (e.g. BindingNode -> anyOf[0] -> fields.items ->
 * AliasType def whose anyOf IS the same array as BindingNode.anyOf). These
 * cycles must be replaced with `$ref` before the schema can be serialised.
 */
export function replaceCircularRefs(
...
function walkAndReplace(
    ...
    for (const key of Object.keys(obj)) {
        ...
        if (ancestors.has(val as object)) {
            const name = resolveRefName(val as object, objToName, arrayToName);
            if (name) {
        ...
        if (Array.isArray(val)) {
            ...
            for (let i = 0; i < val.length; i++) {
                ...
                if (ancestors.has(item as object)) {
                    const name = resolveRefName(item as object, objToName, arrayToName);
...
/**
 * Deep-clone a schema definition, replacing any circular JS object
 * references with JSON Schema `$ref` pointers.  This produces an
 * independent, acyclic copy safe for `JSON.stringify`.
 */
